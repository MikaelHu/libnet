#pragma once
//File: MD5.h
//Description: an implemetation of MD5 algrithm.
//Referance: http://www.cnblogs.com/TenosDoIt/archive/2013/06/02/3114056.html
...
namespace Http {
    namespace Alg {
        ...
        private:
            ...
            void UcharToUint(uint32 output[], const uchar8 input[], const uint32 transLength);
            void FourRound(const uchar8 block[]);
            void LinkResult();

        private:
            uint32 A{ 0x67452301 }, B{ 0xefcdab89 }, C{ 0x98badcfe }, D{ 0x10325476 };
            constexpr static int blockLen_ = 64;    //512/8
            //the remain after last updata (because md5 may be computed segment by segment)
            uchar8 remain_[blockLen_]{ 0 };
            int remainNum_{ 0 };                    //the number of remain_, < 64
            uint64 totalInputBits_{ 0 };
            uchar8 md5Result_[16]{ 0 };             //bit style md5 result,totally 128 bit
            char md5ResultHex_[33]{ 0 };            //hexadecimal style result; md5Result_hex_[32]='\0'
            bool isDone_{ false };                  //indicate the comput is finished;
        };
        ...
        void MD5::Reset(){
            ...
            memset(remain_, 0, blockLen_ * sizeof(uchar8));
            remainNum_ = 0;
            totalInputBits_ = 0;
            memset(md5Result_, 0, 16 * sizeof(uchar8));
            memset(md5ResultHex_, 0, 33 * sizeof(char));
            isDone_ = false;
        }
        ...
        void MD5::LinkResult(){
            ...
            for (int i = 0; i < 16; i++)
                sprintf_s(&md5ResultHex_[i * 2], 33 - i * 2, "%02x", md5Result_[i]);
            md5ResultHex_[32] = '\0';
        }
        ...
        void MD5::printMd5(){
            if (!isDone_)
                std::cout << "Error: computation is not finished" << std::endl;
            else
                std::cout << "MD5 Value: " << md5ResultHex_ << std::endl;
        }
        ...
        std::string MD5::ToString(){
            ...
            std::string a((const char*)md5ResultHex_);

            Reset();
            return a;
        }

        void MD5::UcharToUint(uint32 output[], const uchar8 input[], const uint32 transLength){
            for (uint32 i = 0, j = 0; j < transLength; i++, j += 4){
        ...
        void MD5::UpdateMd5(const uchar8 input[], const int length){
            isDone_ = false;
            totalInputBits_ += ((uint64)length << 3);
        ...
        void MD5::Finalize(){
            ...
            if (temp > 0){
                ...
                totalInputBits_ -= ((uint64)temp << 3);
            }
            else if (temp < 0){
                UpdateMd5(padding, 64 + temp);
                totalInputBits_ -= ((64 + (uint64)temp) << 3);
            }
